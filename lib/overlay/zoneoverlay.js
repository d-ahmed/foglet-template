...
module.exports = class ZoneOverlay extends TMAN {
    ...
    setTimeout(() => {
      this._manager.overlay('tman').communication.onUnicast((id, message) => { 
        if(message.type === 'MUpdateCache'){
          ...
          this._leaders.forEach((leader, idCible)=>{
            if(leader.id === message.peer){
              this._leaders.set(idCible, message.descriptor)
            }
          })
        }
      })
    ...
    if(
      ...
      ((_.intersection(neighbour.descriptor.cibles, descriptorA.cibles).length>0))
        ...
        if(  this._rps.options.maxPeers < this.rps._sampleSize() && !this.myneighbours.has(descriptorA.id)){
          ...
          console.log('neighbour ', neighbour.descriptor, 'descriptorA ',  descriptorA, 'descriptorB ', descriptorB)
          console.log('Inrement maxPeers for ', this._rps.options.descriptor.id, ' and max peers is ', this._rps.options.maxPeers)
          console.groupEnd()*/
  ...
  /*_rankPeers(neighbour, descriptorA, descriptorB, peerA, peerB) {
    // console.log(neighbour, descriptorA, descriptorB)
    if(
      (this._rps.options.descriptor.id === neighbour.descriptor.id) &&
      neighbour.descriptor.cibles.length>0
      
    ){
      if(((_.intersection(neighbour.descriptor.cibles, descriptorA.cibles).length>0))){
        if(!this.myneighbours.has(descriptorA.id) && (this._rps.options.maxPeers <= this.rps._sampleSize())){
          this._rps.options.maxPeers =  this._rps.options.maxPeers+1
          this.myneighbours.add(descriptorA.id)
        }
        
        return -1;
      }else if(((_.intersection(neighbour.descriptor.cibles, descriptorB.cibles).length>0))){
        if(!this.myneighbours.has(descriptorB.id) && (this._rps.options.maxPeers <= this.rps._sampleSize())){
          this._rps.options.maxPeers =  this._rps.options.maxPeers+1
          this.myneighbours.add(descriptorB.id)
        }
        return 1;
      }
      ...
      (this._rps.options.descriptor.id === neighbour.descriptor.id))
    {
      ...
      if(
        ...
        this.myneighbours.has(descriptorB.id) &&
        ((_.intersection(neighbour.descriptor.cibles, descriptorB.cibles).length===0))
      ){
        this._rps.options.maxPeers =  this._rps.options.maxPeers-1
        this.myneighbours.delete(descriptorB.id)
      }
