...
module.exports = class ZoneOverlay extends TMAN {
  ...
  removeCible(id){
    ...
    this.myCybles = this.myCybles.filter(cid=>cid.id!==id)
  }
  ...
  calculateCible () {
      Array.from(this.cibles.values(), (cible)=>{
        if(this._isNearby(cible, this._rps.options.descriptor, cible.perimeter)){
          if(this.myCybles.filter(cid=>cid.id===cible.id).length<=0){
            this.myCybles.push(cible);
          }
        }else{
          if(this.myCybles.filter(cid=>cid.id===cible.id).length>0){
            this.myCybles = this.myCybles.filter(cid=>cid.id!==cible.id)
          }
        }
      })
  }
  ...
  _needConnectionTo(descriptor){
    let trouve = false;
    let i = 0;
    while(i<this.myCybles.length && !trouve){
      let cible = this.myCybles[i];
