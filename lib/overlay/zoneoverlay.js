...
module.exports = class ZoneOverlay extends TMAN {
  ...
  _rankPeers(neighbour, descriptorA, descriptorB, peerA, peerB) {
    ...
    // console.log(this.cibles.values())
    Array.from(this.cibles.values(), (cible)=>{
      const distanceCN = getDistance(cible, this._rps.options.descriptor)
      
      if(descriptorA.id !== this._rps.options.descriptor.id
        && descriptorB.id !== this._rps.options.descriptor.id
        && distanceCN<=cible.perimettre){
        if(this._rps.options.descriptor.cibles.indexOf(cible.id)===-1)this._rps.options.descriptor.cibles.push(cible.id)
        const distanceCA = getDistance(cible, descriptorA)
        if(distanceCA<=cible.perimettre){
          if(!this.myneighbours.has(descriptorA.id)){
            this.myneighbours.add(descriptorA.id)
            this.options.maxPeers =  this.options.maxPeers+1
          }
        }
        const distanceCB = getDistance(cible, descriptorB)
        if(distanceCB<=cible.perimettre){
          if(!this.myneighbours.has(descriptorB.id)){
