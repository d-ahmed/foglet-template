...
class Template extends EventEmitter {
  ...
  sendOverlayUnicastAll(message, overlay) {
    this.neighboursOverlay(overlay).forEach(peer => {
      this.sendOverlayUnicast(overlay, peer, message).then().catch(
          e => {
  ...
  onUnicast(overlay){
    return this.foglet.overlay(overlay).communication.onUnicast;
  }
  ...
  leaveOverlay(overlay, target) {
    this.foglet._networkManager.overlay(overlay)._network.rps._stop()
