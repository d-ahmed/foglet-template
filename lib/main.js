...
class Template extends EventEmitter {
      ...
      foglet: {
        ...
        rps: {
          ...
          options: {
            ...
            pendingTimeout: 1 * 1000, // time before the connection timeout in neighborhood-wrtc
  ...
  changeLocation(x,y){
    ...
    /*let parentIN = this.foglet.overlay()._network.getReachableNeighbours(1000)
    let parentOT = this.foglet.overlay()._network.getNeighbours(1000)

    parentIN.forEach((peer)=>{
      this.foglet.overlay('tman')._network.rps.connect(peer)
    })

    parentOT.forEach((peer)=>{
      this.foglet.overlay('tman')._network.rps.connect(peer)
    })*/
