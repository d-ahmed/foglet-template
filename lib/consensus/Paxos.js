...
class Paxos {
  ...
  prepare() {
    ...
    this.periodic = setInterval(() => {
      ...
      this.progression.maxPeers = this.candidate.template.neighboursOverlay(this.overlay).length + 1

      const ballot = this.progression.ballot.increment();

      const message = new Message(Message.types().PREPARE, { ballot, maxPeers: this.progression.maxPeers});
  ...
  propose(id, message) {
    ...
    if(this.progression.proposed.length >= Math.floor((this.progression.maxPeers / 2) + 1)){
      let value = null;
      if(this.isNoValue()){
        // TODO
        value = 'initial value'
