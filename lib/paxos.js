...
class Paxos {
    ...
    initiateProposal(message){
        if(this.leader.leaderOfCible.get(message.NAME).peer===this.template.foglet.inViewID){
            if(!this.isProssesing.hasOwnProperty(message.NAME)){
                ...
                this.NUM_PEERS[message.NAME] = this.template.neighboursOverlay(message.NAME).length
    ...
    getCible(cible){
        ...
        this.template.foglet.overlay(cible).communication.sendUnicast(
            leader,
            new Message(this.template.foglet.inViewID, messages.MSGET_CIBLE, cible, null, null, {
                perimeter: cible,
    ...
    handlePrepareRequest(message){
        ...
        if(this.greaterThan(message.BALLOT_NUM, this.BALLOT_NUM[message.NAME])){
            ...
            this.template.foglet.overlay(message.NAME).communication.sendUnicast(
                to,
                message
            ).then().catch(
                e => {
