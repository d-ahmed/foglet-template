...
class Leader{
    ...
    doLeaderElection(){
        ...
        let visibleOverlay = Array.from(this.template.foglet._networkManager._overlays.keys());
        let overlays = Array.from(this.template.foglet._networkManager._overlays.values())
        overlays.forEach((network_, index) =>{
            ...
            let cible = this.template.targets.filter(target=> target.id===visibleOverlay[index])
